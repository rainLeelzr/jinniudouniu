...
(function(win) {
	...
	*/
    ...
    //转换日期格式
    jeDt.parse = function(ymd, jns, format) {
        ymd = ymd.concat(jns);
    ...
    InitDate.prototype = {
        ...
        setHtml:function(opts, self) {
            ...
            var botjnsStr = !isformat ? '<ul class="botflex jedatejns"><li><em data-jns="24"></em><i>:</i></li><li><em data-jns="60"></em><i>:</i></li><li><em data-jns="60"></em></li></ul>' + '<div class="botflex jedatebtn"><span class="jedateclear" style="width:31%;">清空</span><span class="jedatetodaymonth" style="width:31%;">今天</span><span class="jedateok" style="width:31%;">确认</span></div>' :'<div class="botflex jedatebtn"><span class="jedateclear" style="width:31%;">清空</span><span class="jedatetodaymonth" style="width:31%;">本月</span><span class="jedateok" style="width:31%;">确认</span></div>';
            var datebotStr = '<div class="jedatebot">' + botjnsStr + "</div>";
            var dateHtmStr = isformat ? datetopStr + dateymList + datebotStr :datetopStr + dateymList + dateriList + datebotStr + '<div class="jedatepropjns"></div>';
            jeDt.html(QD(Cell)[0], dateHtmStr);
            opts.isClear ? "" :QD(Cell + " .jedatebot .jedateclear")[0].style.display = "none";
            if (opts.isTime) {
                var djnsArr = jeDt.val(self) != "" || jeDt.text(self) != "" ? [ Tms[3], Tms[4], Tms[5] ] :[ date.getHours(), date.getMinutes() + 1, date.getSeconds() ];
                jeDt.each(QD(Cell + " .jedatebot .jedatejns em"), function(i, cls) {
                    jeDt.html(cls, jeDt.digit(djnsArr[i]));
                });
            } else {
                if (!isformat) QD(Cell + " .jedatebot .jedatejns")[0].style.display = "none";
        ...
        events:function(that, opts, self, Tms) {
            ...
            if (!isformat) {
                ...
                jeDt.each(QD(Cell + " .jedatebot .jedatejns em"), function(i, cls) {
                    jeDt.on(cls, "click", function() {
                        var jnsStr = "", acton, jnscell = QD(Cell + " .jedatepropjns")[0], jnslen = jeDt.attr(cls, "data-jns"), jnsstxt = [ "小时", "分钟", "秒数" ], 
						removeEmpty = function() {
                            jeDt.removeClass(jnscell, jnslen == 24 ? "jedateh" :"jedatems");
                            jeDt.html(jnscell, "");
                        };
                        jnsStr += '<div class="jedatejnstitle">' + jnsstxt[i] + '<div class="jedatejnsclose">&times;</div></div>';
                        for (var h = 0; h < jnslen; h++) {
                            h = jeDt.digit(h);
                            acton = jeDt.text(cls) == h ? "action" :"";
                            jnsStr += '<p class="' + acton + '">' + h + "</p>";
                        }
                        jeDt.removeClass(jnscell, jnslen == 24 ? "jedatems" :"jedateh").addClass(jnscell, jnslen == 24 ? "jedateh" :"jedatems");
                        jeDt.html(jnscell, jnsStr);
                        jeDt.each(QD(Cell + " .jedatepropjns p"), function(i, p) {
                            jeDt.on(p, "click", function() {
                                jeDt.html(cls, jeDt.digit(jeDt.text(p)));
                                removeEmpty();
                            });
                        });
                        jeDt.each(QD(Cell + " .jedatepropjns .jedatejnstitle"), function(i, c) {
            ...
            jeDt.on(QD(Cell + " .jedatebot .jedateok")[0], "click", function(ev) {
                ...
                if (!isformat) {
                    ...
                    jeDt.each(QD(Cell + " .jedatejns em"), function(l, tval) {
